...
def rank_reveal_func(window, green, red, Rank_Reveal) :

    #Si la couleur du boutton == rouge alors on la change en vert
    #Et vice-versa
    curent_cl = Rank_Reveal.cget('bg')
    if curent_cl == red :
        #On appelle le rr
        multiprocessing.freeze_support()
        t_rr = Process(target = rr)
        global t_rr
        t_rr.start()
        return green
    elif curent_cl == green :
...
def buttons(window) :
    ...
    #Rank Reveal
    Rank_Reveal = Button(window, text="Rank Reveal", bg=red, fg='#000000', font=(40), command= lambda: Rank_Reveal.configure(background = rank_reveal_func(window, green, red, Rank_Reveal)))
