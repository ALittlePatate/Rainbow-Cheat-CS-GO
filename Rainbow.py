...
def config_loader() :
    ...
    def file_path_conf() :          #Sous programme qui récupère le chemain d'accès de la config
        config = askopenfilename()
        MessageBox = ctypes.windll.user32.MessageBoxW
        MessageBox(None, 'Config Loaded !', 'Success', 0)
        global config
    
    def path_writer() :                 #Sous programme qui note le chemain d'accès dans configs/path.txt
        config = optmenu.get()
        f = open("configs/path.txt", "w")
        f.write(config)
        f.close()
        MessageBox = ctypes.windll.user32.MessageBoxW
        MessageBox(None, 'Config Used !', 'Success', 0)
...
def triggerbot_conf() :
    ...
    label1.place(relx=1, x=-150, y=0, anchor=NE)                                  #Délai (titre)
    tg_entry = Entry(triggerbot_conf, textvariable = delay_tg, width=25)          #Délai (entrée)
    tg_entry.place(relx=1, x=-145, y=25, anchor=NE)                               #Délai (entrée)
    ...
    bouton.place(relx=1, x=-80, y=20, anchor=NE)                                                                                       #Délai (boutton)
    label2 = Label(triggerbot_conf, text = 'Keybind :', font=(40))                #Keybind (titre)
    label2.place(relx=1, x=-231, y=60, anchor=NE)                                 #Keybind (titre)
    tg_entry2 = Entry(triggerbot_conf, textvariable = delay_tg2, width=25)        #Keybind (entrée)
    tg_entry2.place(relx=1, x=-145, y=90, anchor=NE)                              #Keybind (entrée)
    ...
    bouton2.place(relx=1, x=-80, y=85, anchor=NE)                                                                                         #Keybind (bouton)
